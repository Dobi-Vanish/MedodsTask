...
func (s *RewardService) Refresh(w http.ResponseWriter, r *http.Request) { //nolint: funlen
	fmt.Println("DEBUG: Refresh function has been called")
	accessTokenReceived, err := (r.Cookie("accessToken"))
	if err != nil {
		httputils.ErrorJSON(w, err, http.StatusUnauthorized)
	...
	fmt.Println("DEBUG: ExtractGUIDFromToken has been called")
	guid, err := ExtractGUIDFromToken(accessTokenReceived.Value)
	if err != nil {
	...
	fmt.Println("DEBUG: refreshToken is extracting")
	...
	ip := GetClientIP(r)
	fmt.Println("DEBUG: Parsing Access Token")
	AccessClaims, err := s.ParseAccessToken(accessTokenReceived.Value)
	if err != nil {
		httputils.ErrorJSON(w, err, http.StatusUnauthorized)
	...
	if AccessClaims.IP != ip {
		s.sendIPChangeWebhook(AccessClaims.GUID, AccessClaims.IP, ip)
	}
	fmt.Println("DEBUG: Checking for User Agent")
	if AccessClaims.UserAgent != r.UserAgent() {
		s.Logout(w, r)
		fmt.Println("User agent has been changed, forced logout")

		return
	}
	fmt.Println("DEBUG: Validating Refresh Token")
	ok, err := s.Repo.ValidateRefreshToken(refreshCookie.Value, AccessClaims.PairID, guid)
	if err != nil {
		s.Logout(w, r)
		httputils.ErrorJSON(w, err, http.StatusUnauthorized)
	...
	tokenService := token.NewTokenService()
	fmt.Println("DEBUG: Generating new tokens")
	accessToken, refreshToken, hashToken, err := tokenService.GenerateTokensWithPairID(ip, r.UserAgent(), guid, AccessClaims.PairID)
	if err != nil {
		httputils.ErrorJSON(w, err, http.StatusInternalServerError)

		return
	}
	fmt.Println("DEBUG: Updating Refresh token")
	err = s.Repo.UpdateRefreshToken(guid, AccessClaims.PairID, hashToken)
	if err != nil {
		httputils.ErrorJSON(w, err, http.StatusInternalServerError)

		return
	}
	fmt.Println("DEBUG: Setting new cookies")
	s.setAuthCookies(w, accessToken, refreshToken)
...
func (s *RewardService) sendIPChangeWebhook(guid, oldIP, newIP string) {
	...
	defer func(Body io.ReadCloser) {
		err := Body.Close()
		if err != nil {
			log.Printf("Failed to finish sending error message: %v", err)
